...
| DR-067 | Pending Validate data is staged in `SourceJob.extracted_metadata['structure']` JSONB, NOT in a separate `pending_facts` / `pending_objects` table. The Validate API reads facts / objects / disambiguation_pending directly from that JSONB; accepting a fact promotes it into `lucid_facts` ES; discarding it appends the uid to `decided_fact_uids` so the UI hides it. | Beta-scope simplicity — a dedicated table would mirror a JSONB that already carries the same fields, doubling the maintenance surface for zero capability gain. Phase 1+ may revisit if multi-worker contention or analytic-query patterns require it. Adopted 2026-06-01 (feat/lucid-sprint-4b-pr1). |
| DR-068 | Chrome Extension auth — chrome.cookies bridge against the existing `lucid_jwt` + `lucid_space_id` cookies the web app sets at `/login` (Sprint 4A `lib/auth.setToken` / `setCurrentSpace`). The extension reads them via `chrome.cookies.get({ url: WEB_BASE, name })` and never implements its own login flow. | Lowest-friction handoff — postMessage requires a content script + origin verification + web app changes; cookie bridge needs only `host_permissions` + `cookies` permission. Beta non-httpOnly cookies make this safe; Phase 1+ moves to httpOnly + a dedicated OAuth-style exchange. Adopted 2026-06-01 (feat/lucid-sprint-2a-pr1). |
