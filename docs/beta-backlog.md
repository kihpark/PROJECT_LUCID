...
```
Epic 1  Foundation
  P0: 프로젝트 셋업, 데이터 모델, Neo4j 스키마, 기본 API, 인증
...
Epic 3  Structure
  P0: AtomicFact 분해 엔진, Object 추출+매칭, Link 추출, FAISS
  P1: 분해 품질 자동 점검
...
Epic 5  Surface
  P0: On/Off 토글, Active Recall, Passive Recall, Contradiction,
      Gatekeeping, Staleness
  P1: 응답 길이 정책 튜닝, 인용 포맷 정제

Epic 6  Stellar View
  P0: 기본 그래프 렌더링, Elastic 필터링, 모순 시각화
  P1: stale 시각화, 클러스터 라벨, 줌 레벨링
...
Sprint 6D   Surface — Gatekeeping + Staleness     병렬
   ↓
...
```
P0:
  · FastAPI 프로젝트 구조 (AGENTS.md §3 그대로)
  · Neo4j + FAISS Docker Compose
  · Anthropic API 클라이언트 wrapper
...
**완료 조건**
```
✓ docker-compose up 으로 전체 환경 기동
✓ /api/health 응답 정상
✓ Anthropic API 호출 테스트 통과
✓ Neo4j 연결 테스트 통과
✓ FAISS 인덱스 생성·읽기 테스트 통과
✓ pytest 기본 셋업 + 1개 smoke test 통과
```
...
test_neo4j_connection
  Cypher 쿼리 "RETURN 1" 실행 → 결과 1

test_faiss_index_roundtrip
  vector 저장 → 검색 → 동일 벡터 반환
...
### Sprint 1A — Data Models + Neo4j Schema

**목표**
CSVS 명세서의 12개 Object Class와 Link 9종을 Pydantic + Neo4j로 구현.
...
test_neo4j_create_fact_node
  FactNode를 Neo4j에 저장 → uid로 조회 가능
...
P0:
  ...
  · 분해 실패 처리 (정직한 빈 결과)
  · 출처 메타데이터 사실별 분배
  · 시간 메타데이터 (valid_from, valid_until) 추출
...
**목표**
검증된 그래프 시각화. D3 force simulation.
...
**데모 시나리오**
```
1. 50개 FactNode가 있는 Space 열기
2. 별자리 자동 형성 확인
3. "Knowledge" 도메인 필터 → 다른 도메인 후퇴
4. 모순 사실 한 쌍 → 붉은 선 확인
```
...
test_staleness_daily_scan
  valid_until 지난 사실 → is_stale=true
