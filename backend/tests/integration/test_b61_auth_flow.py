...
Covers the three pinned contracts:
  - /me after register returns identity + is_new_user=True + the
...
def test_b61_me_returns_identity_after_register(client, fresh_email):
    """Register → /me returns email, display_name, default_space_id,
    and is_new_user=True (no facts yet)."""
    reg = client.post(
        "/api/auth/register",
        json={
            "email": fresh_email,
    ...
    # Just registered with no captured facts → cold-start.
    assert me["is_new_user"] is True
    assert me["user_id"] == body["user"]["id"]
...
def test_b61_me_returns_200_after_logout_because_jwt_is_stateless(
    ...
    """Register → logout (server-side stateless) → /me with the same
    token still returns 200. This is intentional: JWT has no
    server-side revocation in beta. If a denylist lands in Phase 1+
...
def test_b61_me_default_space_is_first_personal_space(client, fresh_email):
    """When the user has one personal space, default_space_id is that
    space — and it is the same id that /register returned."""
    reg = client.post(
        "/api/auth/register",
        json={"email": fresh_email, "password": "longerthan8chars!"},
    )
    ...
    assert me["default_space_id"] == body["space_id"]
