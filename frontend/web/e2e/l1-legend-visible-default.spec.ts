...
test('L1: STELLAR LEGEND is visible by default with all vocabulary items', async ({
  ...
  // legend itself. ★ 2026-07-01 (PO): WHAT is split into 6 sub-rows
  // (자원/개념/행위/지식/사건/지표) so the user can tell which amber category
  // a node belongs to — 옛 EVENT top-level bucket 폐기 (사건 = WHAT sub).
  ...
  await expect(page.getByTestId('stellar-legend-item-what-concept')).toBeVisible();
  ...
  await expect(page.getByTestId('stellar-legend-item-what-event')).toBeVisible();
  await expect(page.getByTestId('stellar-legend-item-what-metric')).toBeVisible();
